...
let $product=$(`<div class="card col-xs-12 col-sm-4 col-md-3 mx-auto" data-product-id="${id}">`);
	...
	$product.append($(`<span class="product-description">ОПИС:</span>`).text(description));
        ...
        $product.append($(`<button type="button" class="btn button bg-success product-buy  add_item" data-id='${id}' data-title="${name}" data-img="${image_url}" data-price="${special_price}">`).text("В корзину"));
	}else{
        $product.append($(`<span class="mx-auto product-price">`).text("Цена: " + price));
        $product.append($(`<button type="button" class="btn button bg-success product-buy  add_item" data-id='${id}' data-title="${name}" data-img="${image_url}" data-price="${price}">`).text("В корзину"));
    }
	return $product;
};
